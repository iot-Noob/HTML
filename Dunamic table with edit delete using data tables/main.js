...
if(fn==""&&ln==""&&age==""||fn==""||ln==""||age=="") {
   ...
   var btdf=` <button id="btd" name="bta" type="button" class="btn  material-icons  btcs">delete</button>   <button id="bte" name="bte" type="button" class="btn material-icons  btcs">edit</button>`;
     var udt={
     id:index,
     fname:fn,
     lname:ln,
    ...
    cf.row.add([udt.id,udt.fname,udt.lname,udt.agf,udt.bte]).draw(false);
     //$('#mtb').append(dtc);
    $("#Fname").val("");
    $("#Lname").val("");
    $("#age").val("");
...
$("#mtb").on('click','#btd',function(){ //remmove
    ...
    cf.row($(this).parents('tr')).remove().draw(false);
     index--;
});
